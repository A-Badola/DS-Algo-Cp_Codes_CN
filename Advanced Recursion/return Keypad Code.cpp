/*
Given an integer n, using phone keypad find out all the possible strings that can be made using digits of input n.
The numbers and their corresponding codes are given below:
0: ""
1: ""
2: "abc"
3: "def"
4: "ghi"
5: "jkl"
6: "mno"
7: "pqrs"
8: "tuv"
9: "wxyz"
Return empty string for numbers 0 and 1.
Note:
1. The order of strings are not important.
2. The input number will have digits between: [2, 9].
Input Format :
First line of input will contain T number of test cases.
Each input consists of a single line containing an integer n.
Output Format :
For each test case, print all possible strings in different lines.
Constraints :
1 <= T <= 100
1 <= n <= 10^6
Sample Input:
1
23
Sample Output:
ad
ae
af
bd
be
bf
cd
ce
cf
*/

#include<bits/stdc++.h>
using namespace std;

string keypad[10]={"","","abc","def","ghi","jkl","mno","pqrs","tuv","wxyz"};

int solve(int n,string output[]){
    if(n==0||n==1){
        output[0]="";
        return 1;
    }
    int smalln = n/10;
    int remn = n%10;
    if(remn==0||remn==1){
        return 0;
    }
    int smallOutputSize = solve(smalln,output);
    for(int i=1;i<=keypad[remn].size()-1;i++){
      for(int j=0;j<smallOutputSize;j++){
        output[smallOutputSize*i + j] = output[j];
      }
    }
    for(int i=0;i<smallOutputSize;i++){
      for(int j=0;j<keypad[remn].size();j++){
        output[i + smallOutputSize*j] = output[i + smallOutputSize*j] +  keypad[remn][j];
      }
    }
    return smallOutputSize*keypad[remn].size();
}

int main(){
    //freopen("input.txt","r",stdin);
    //freopen("output.txt","w",stdout);
    int t;
    cin>>t;
    while(t--){
      int n;
      cin>>n;
      string* output = new string[17000];
      int totalEl = solve(n,output);
      for(int i=0;i<totalEl;i++){
          cout<<output[i]<<endl;
      }
      delete[] output;
    }
    return 0;
}