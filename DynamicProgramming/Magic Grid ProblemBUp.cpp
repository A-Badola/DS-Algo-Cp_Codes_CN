...
int min_req(int** grid,int r,int c){
  int** mat = new int*[r];
  for(int i=0;i<r;i++){
    mat[i] = new int [c];
    for(int j=0;j<c;j++){
  ...
  for(int i=r-3;i>=0;i--){
    int k = mat[i+1][c-1] - grid[i+1][c-1];
    if(k<1){
      mat[i][c-1] = 1;
    }
    else{
  ...
  for(int i=c-3;i>=0;i--){
    int k = mat[r-1][i+1] - grid[r-1][i+1];
    if(k<1){
      mat[r-1][i] = 1;
    }
    else{
  ...
  for(int i=r-2;i>=0;i--){
    for(int j=c-2;j>=0;j--){
      int k = min(mat[i+1][j] - grid[i+1][j],mat[i][j+1] - grid[i][j+1]);
      if(k<1){
        mat[i][j] = 1;
      }
      else{
