...
int main(){
  ...
  for(int i=1;i<=n;i++){
    for(int j=1;j<=w;j++){
      ...
      if(j>=arr[i-1].second){
        mat[0][i][j] = max(mat[0][i-1][j], mat[0][i-1][j-arr[i-1].second] + arr[i-1].first);
      }
    }
  }
  ...
  for(int p = 1; p<=10; p++){
     ...
     for(int i=1; i<=n; i++){
       for(int j=1; j<=w; j++){
         mat[flag][i][j] = mat[flag][i-1][j];
         if(j>=arr[i-1].second){
           mat[flag][i][j] = max(mat[flag][i][j], max( mat[flag][i-1][j-arr[i-1].second] + arr[i-1].first, mat[flag^1][i-1][j-arr[i-1].second] + arr[i-1].first*prime[p]));
         }
       }
     }
  }
